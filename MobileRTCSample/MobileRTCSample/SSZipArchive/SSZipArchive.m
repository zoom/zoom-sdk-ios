      ...
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    ...
    do {
        @autoreleasepool {
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveWillUnzipFileAtIndex:totalFiles:archivePath:fileInfo:fileName:)]) {
                if(![delegate zipArchiveWillUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                                 archivePath:path fileInfo:fileInfo fileName:strPath]){
                    unzCloseCurrentFile(zip);
                    ret = unzGoToNextFile(zip);
                    free(filename);
                    continue;
            ...
            BOOL hasDirTraversal = [strPath rangeOfString:@"../"].location != NSNotFound || [strPath rangeOfString:@"/.."].location != NSNotFound;
            if (hasDirTraversal) {
                // ignoring item: directory traversal
                NSLog(@"[SSZipArchive] Security Ignore Item (Dir Traversal): %@", destination);
                unzCloseCurrentFile(zip);
                ret = unzGoToNextFile(zip);
                free(filename);
                continue;
            ...
            if (!fileIsSymbolicLink) {
                ...
                if (fp) {
                    ...
                    BOOL isUsePermission = YES;
                    if ([delegate respondsToSelector:@selector(zipArchiveGetUnzipFilePermissionAtIndex:totalFiles:archivePath:fileInfo:fileName:)]) {
                        if(![delegate zipArchiveGetUnzipFilePermissionAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                                         archivePath:path fileInfo:fileInfo fileName:strPath])
                            isUsePermission = NO;
                    }
                    
                    if (permissions != 0 && isUsePermission) {
